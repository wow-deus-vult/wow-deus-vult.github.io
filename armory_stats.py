...
# ── PvE / PvP build classifier ───────────────────────────────────────────────
# Signature PvP talents per class (a PvE build practically never takes these)
PVP_TALENTS = {
    "WA": {"Second Wind", "Iron Will", "Improved Hamstring"},
    "PA": {"Divine Purpose", "Eye for an Eye", "Improved Hammer of Justice",
           "Guardian's Favor"},
    "HU": {"Improved Deterrence", "Improved Wing Clip"},
    "RO": {"Cheat Death", "Improved Kick", "Improved Sprint", "Improved Kidney Shot"},
    "PR": {"Psychic Horror", "Improved Psychic Scream", "Reflective Shield"},
    "DK": {"On a Pale Horse", "Improved Unholy Presence"},
    "SH": {"Astral Shift", "Improved Grounding Totem", "Earthen Power"},
    "MA": {"Improved Counterspell", "Prismatic Cloak", "Frozen Core"},
    "WK": {"Soul Link", "Improved Howl of Terror", "Demonic Resilience"},
    "DR": {"Brutal Impact", "Owlkin Frenzy", "Improved Barkskin"},
}
...
def _talents_db():
    ...
    if _TALENTS_DB is None:
        path = os.path.join(_HERE, "armory", "talents.json")
        try:
...
def classify_pvx(class_code, talent_ranks, stats, equip, items_cache):
    """'pvp' / 'pve' from resilience, Gladiator items and signature talents."""
    score = 0
    res = (stats or {}).get("res", 0)
    if res >= 100:  score += 2
    elif res >= 40: score += 1

    glad = 0
    for slot in equip or []:
        iid = slot.get("itemId")
        if not iid:
            continue
        nm = (items_cache.get(str(iid), {}) or {}).get("name", "")
        if "Gladiator" in nm or "гладиатор" in nm.lower():
            glad += 1
    if glad >= 4:  score += 2
    elif glad >= 2: score += 1

    markers = PVP_TALENTS.get(class_code, set())
    trees = _talents_db().get(class_code)
    if trees and talent_ranks and markers:
        ...
        for ti, tree in enumerate(trees):
            ...
            for i, t in enumerate(tree.get("talents", [])):
                if i < len(s) and t.get("name") in markers:
                    pts += int(s[i]) if s[i].isdigit() else 0
        if pts >= 3:  score += 2
        elif pts >= 1: score += 1

    return "pvp" if score >= 2 else "pve"
